...
the shared MicroPython runtime (`cameras/common/micropython`) rather than
...
Sibling: `cameras/xiao_pantilt/PLAN.md` (ESP-IDF); the two must agree on
the contract (§9 of the operation spec), not on code.

## 0. Runtime decision: MicroPython, shared runtime

The N6 is live on `cameras/common/micropython` (2.0.10-n6). Everything the
...
`cameras/common/espidf/components/dusty_core/src/night.c`.
...
- **A** (sonnet): common modules night/rank/gamespool/persist/led/judge/
  ...
  + host tests `cameras/common/tests/test_game.py`.
