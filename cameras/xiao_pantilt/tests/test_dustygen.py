...
tools/dustygen). No board, no network, no real secrets are read or written.
    ...
    python3 -m unittest discover -s cameras/xiao_pantilt/tests -v
"""
import json
import os
import re
...
REPO = HERE.parents[2]                      # .../tests -> xiao_pantilt -> cameras -> repo root
DUSTYGEN = REPO / 'tools' / 'dustygen'
REAL_MANIFEST = REPO / 'cameras' / 'xiao_pantilt' / 'camera.toml'
