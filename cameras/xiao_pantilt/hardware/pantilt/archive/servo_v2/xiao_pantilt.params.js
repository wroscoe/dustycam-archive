// xiao_pantilt viewer sidecar (servo v2): pan/tilt pose + derived gear spins.
// Mirrors pantilt_lib.py (same axes, same mesh phase). Viewer-time only.
const Z_TILT = 74.0;
const PAN_RATIO = 26 / 30, TILT_RATIO = 28 / 18;   // gear teeth / pinion teeth
const PAN_PINION = [-28.15, 0, 0];
const TILT_PINION = [0, 0, 50.85];   // world at pan 0, straight below the tilt axis
export default {
  manifest: {
    schemaVersion: 1,
    parameters: {
      ...
      tilt_deg: { type: "number", label: "Tilt", unit: "deg", min: -30, max: 60, step: 1, default: 0,
                  description: "Rotates cradle + camera about the lens axis (+ = look up); tilt pinion spins 1.56x the other way." },
    },
    features: {
      yoke_group: { ref: "#o1.2", label: "Yoke (pans)" },
      tilt_group: { ref: "#o1.2.6", label: "Cradle (tilts)" },
