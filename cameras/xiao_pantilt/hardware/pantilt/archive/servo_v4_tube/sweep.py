...
def static_checks():
    ...
    out.append(("pod max radius vs ring opening", round(L.ring_r_in - rmax, 2), "mm", L.ring_r_in - rmax >= 1.0))
    ...
    usb = cam.bounding_box()
    out.append(("USB plug space under the board", round(L.Z_BOARD0 - L.Z_GEAR_TOP, 1), "mm (>= 12 straight plug)", L.Z_BOARD0 - L.Z_GEAR_TOP >= 12))
    ...
    out.append(("pan pinion tip vs well wall", round(L.well_r + pp.min.X, 2), "mm", L.well_r + pp.min.X >= 1.0))
...
def main():
    ...
    with open(a.out, "w") as f:
        f.write(f"# xiao_pantilt interference sweep (v4 pan-only, tube)\n\nbacklash {L.gear_backlash_mm} mm, {len(poses)} poses, "
