...
/* 1 if the sensor is currently initialised (esp_camera_init() has run and
 * xc_cam_deinit() hasn't since). Lets a caller like contact.c's
 * cam_ensure() decide whether it needs to init the camera itself or can
 * just reuse a session someone else already opened. */
