...
{
    /* Review finding (MAJOR, camera lifecycle): callers now include
     * contact.c's cam_ensure()/cam_finish() wrapper (a hook may need the
     * camera during a BLE-phase window that hasn't opened it yet, or may
     * find it already open from the caller's wake cycle) -- guard against
     * a double esp_camera_init() here rather than trusting every call
     * site to check first (the sarg lesson this whole file's header
     * comment cites is specifically about calling this without a deinit
     * first). */
