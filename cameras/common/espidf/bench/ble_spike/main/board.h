 ...
 *
 * Copied verbatim from cameras/xiao_pantilt/software/app/main/board.h for
 * the dusty_ble bench spike (docs/phone_app_plan.md §6 P0): the spike
 * flashes the same physical XIAO board, so the pin map is identical. Keep
 * these two files in sync by hand until there is a shared per-board
 * header worth factoring out.
 */
