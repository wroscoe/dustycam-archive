...
same physical board as `cameras/xiaocam1` (Seeed XIAO ESP32S3 Sense);
...
`cameras/xiaocam1/software/app/` (verified to resolve correctly
...
```sh
cd runtime/espidf/bench/ble_spike
make build
```
...
`cameras/xiaocam1/software/app/sdkconfig.secrets` directly (see
